...
PI_URL = "http://192.168.50.149:8000"
...
class EyeTrackerGUI:
    ...
    def __init__(self, root):
        ...
        # Streams and Video Writer handles
        self.eye_stream = None
    ...
    def _start_tracking_thread(self):
        """Spawns background thread for processing streams without blocking UI."""
    ...
    def _tracking_loop(self):
        ...
        self.eye_stream = cv2.VideoCapture(PI_URL + "/eye_feed")
        self.world_stream = cv2.VideoCapture(PI_URL + "/world_feed")
        ...
        while self.is_running:
            ret_eye, eye_frame = self.eye_stream.read()
            ret_world, world_frame = self.world_stream.read()
            ...
            # Rescale eye stream to match world height for seamless side-by-side video
            h_w, w_w = world_frame.shape[:2]
            h_e, w_e = eye_frame.shape[:2]
            scale_ratio = h_w / float(h_e)
            eye_resized = cv2.resize(eye_frame, (int(w_e * scale_ratio), h_w))
            ...
            img_eye_rgb = cv2.cvtColor(
                cv2.resize(eye_frame, (display_w, display_h)), cv2.COLOR_BGR2RGB
            )
    ...
    def on_closing(self):
        ...
        self.is_running = False
        if self.eye_stream is not None:
            self.eye_stream.release()
        if self.world_stream is not None:
            self.world_stream.release()
        self.root.destroy()
