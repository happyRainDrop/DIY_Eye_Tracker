...
def rotate_image(image, angle):
    # Rotate an image "angle" degrees ccw
...
def process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame, darkest_point, debug_mode_on, render_cv_window, no_renders=False):
    global ray_lines
    global max_rays
...
def process_frame(frame, no_renders = False):
    ...
    final_rotated_rect = process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame, darkest_point, False, False, no_renders)
    
    return final_rotated_rect

###### ADDED FOR CALIBRAION
def get_3d_gaze_vector(frame):
    """
    Processes a single eye frame using the Orlosky method and extracts the 
    resulting 3D gaze direction vector.
    
    Parameters:
        frame (np.ndarray): The raw BGR input image frame from the eye camera.
        
    Returns:
        np.array([x_dir, y_dir, z_dir]): Normalized 3D direction vector, 
                                         or None if tracking fails.
    """
    # 1. Run the existing Orlosky thresholding & contour pipeline.
    # Note: process_frame handles cropping, thresholding levels, contour optimization,
    # and tracking updates automatically.
    final_rotated_rect = process_frame(frame, no_renders = True)
    
    # 2. Extract the tracked details via your global tracking result dictionary.
    tracking_result = get_last_tracking_result()
    
    # 3. If tracking fails or an ellipse wasn't successfully fit, return None.
    if tracking_result is None or tracking_result["pupil_ellipse"] is None:
        return None
        
    # 4. Extract parameters required to compute the 3D gaze vector
    # x, y = Pupil ellipse center screen coordinates
    # center_x, center_y = Eye sphere center screen coordinates
    x, y = tracking_result["pupil_ellipse"]["center"]
    center_x, center_y = tracking_result["eye_center"]
    
    # 5. Call your existing compute_gaze_vector function to solve for the 3D details.
    # We enforce screen width and height matching your fixed 640x480 aspect crop.
    sphere_center, gaze_rotated = compute_gaze_vector(
        x, y, 
        center_x, center_y, 
        screen_width=640, 
        screen_height=480
    ...
    # 6. Return the normalized 3D direction vector if computation succeeds
    if gaze_rotated is not None:
        return np.array(gaze_rotated)
